...
public class FokRLPValue implements ActorService {
    ...
    public static FokRLPValue fromRlpEncoded(byte[] data) {

        if (data != null && data.length != 0) {
            FokRLPValue v = new FokRLPValue();
            v.init(data);
            return v;
        }
        return null;
    ...
    public FokRLPValue(Object obj) {

        this.decoded = true;
        if (obj == null) return;

        if (obj instanceof FokRLPValue) {
            this.value = ((FokRLPValue) obj).asObj();
        } else {
            this.value = obj;
        }
    }

    public FokRLPValue withHash(byte[] hash) {
        sha3 = hash;
        return this;
    }
    ...
    public byte[] asBytes() {
        ...
        return BytesHelper.EMPTY_BYTE_ARRAY;
    }
    ...
    public FokRLPValue get(int index) {
        if (isList()) {
            // Guard for OutOfBounds
            if (asList().size() <= index) {
                return new FokRLPValue(null);
            }
            if (index < 0) {
                throw new RuntimeException("Negative index not allowed");
            }
            return new FokRLPValue(asList().get(index));
        }
    ...
    public void decode(){
        if (!this.decoded) {
            this.value = FokRLP.decode(rlp, 0).getDecoded();
            this.decoded = true;
        }
    }
    ...
    public byte[] encode() {
        if (rlp == null)
            rlp = FokRLP.encode(value);
        return rlp;
    }

    public byte[] hash(){
//        if (sha3 == null)
//            sha3 = encApi.sha3Encode(encode());
        return sha3;
    }

    public boolean cmp(FokRLPValue o) {
    ...
    public int countBranchNodes() {
        ...
        if (this.isList()) {
            ...
            for (Object obj : objList) {
                i += (new FokRLPValue(obj)).countBranchNodes();
            }
            return i;
