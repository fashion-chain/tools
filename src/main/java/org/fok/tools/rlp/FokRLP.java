...
import org.fok.tools.bytes.BytesDecodeResult;
...
public class FokRLP {
	 ...
	 */
	public static FokRLPList decode2(byte[] msgData) {
		FokRLPList rlpList = new FokRLPList();
		fullTraverse(msgData, 0, 0, msgData.length, 1, rlpList);
		return rlpList;
	}

	public static FokRLPElement decode2OneItem(byte[] msgData, int startPos) {
		FokRLPList rlpList = new FokRLPList();
		...
		try {
			...
			while (pos < endPos) {
				...
				if ((msgData[pos] & 0xFF) > OFFSET_LONG_LIST) {
					...
					FokRLPList newLevelList = new FokRLPList();
				...
				if ((msgData[pos] & 0xFF) > OFFSET_LONG_ITEM && (msgData[pos] & 0xFF) < OFFSET_SHORT_LIST) {
					...
					FokRLPItem rlpItem = new FokRLPItem(item);
					rlpList.add(rlpItem);
					pos += lengthOfLength + length + 1;
				...
				// null item
				if ((msgData[pos] & 0xFF) == OFFSET_SHORT_ITEM) {
					byte[] item = BytesHelper.EMPTY_BYTE_ARRAY;
					FokRLPItem rlpItem = new FokRLPItem(item);
					rlpList.add(rlpItem);
					pos += 1;
					continue;
	 ...
	 */
	public static BytesDecodeResult decode(byte[] data, int pos) {
		if (data == null || data.length < 1) {
			return null;
		}
		int prefix = data[pos] & 0xFF;
		if (prefix == OFFSET_SHORT_ITEM) { // 0x80
			return new BytesDecodeResult(pos + 1, ""); // means no length or 0
		} else if (prefix < OFFSET_SHORT_ITEM) { // [0x00, 0x7f]
			return new BytesDecodeResult(pos + 1, new byte[] { data[pos] }); // byte
																		// is
																		// its
																		// own
																		// RLP
			...
			return new BytesDecodeResult(pos + 1 + len, copyOfRange(data, pos + 1, pos + 1 + len));
			...
			int lenbytes = byteArrayToInt(copyOfRange(data, pos + 1, pos + 1 + lenlen)); // length
																							...
																							// encoded
																							// bytes
			return new BytesDecodeResult(pos + 1 + lenlen + lenbytes,
	...
	private static BytesDecodeResult decodeList(byte[] data, int pos, int prevPos, int len) {
		List<Object> slice = new ArrayList<>();
		for (int i = 0; i < len;) {
			// Get the next item in the data list and append it
			BytesDecodeResult result = decode(data, pos);
		...
		return new BytesDecodeResult(pos, slice.toArray());
	}
...
//	}
	...
	private static byte[] toBytes(Object input) {
		if (input instanceof byte[]) {
			...
			return (inputLong == 0) ? BytesHelper.EMPTY_BYTE_ARRAY : asUnsignedByteArray(BigInteger.valueOf(inputLong));
		} else if (input instanceof Integer) {
			Integer inputInt = (Integer) input;
			return (inputInt == 0) ? BytesHelper.EMPTY_BYTE_ARRAY : asUnsignedByteArray(BigInteger.valueOf(inputInt));
		} else if (input instanceof BigInteger) {
			BigInteger inputBigInt = (BigInteger) input;
			return (inputBigInt.equals(BigInteger.ZERO)) ? BytesHelper.EMPTY_BYTE_ARRAY : asUnsignedByteArray(inputBigInt);
		} else if (input instanceof FokRLPValue) {
			FokRLPValue val = (FokRLPValue) input;
