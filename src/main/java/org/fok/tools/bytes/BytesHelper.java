...
public class BytesHelper {
	...
	/**
	 * Parses fixed number of bytes starting from {@code offset} in
	 * {@code input} array. If {@code input} has not enough bytes return array
	 * will be right padded with zero bytes. I.e. if {@code offset} is higher
	 * than {@code input.length} then zero byte array of length {@code len} will
	 * be returned
	 */
	 ...
	 * right-padded with zero bytes if there is not enough bytes in
	 * {@code input}
	 *
	 * @param idx
